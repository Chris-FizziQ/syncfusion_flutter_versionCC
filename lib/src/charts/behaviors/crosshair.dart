...
class CrosshairBehavior extends ChartBehavior {
  ...
  /// Controls whether the crosshair is visible or not.
  ///
  /// This property can be used to explicitly control the visibility
  /// of the crosshair. When set to true, the crosshair will be visible
  ...
  void hide() {
    // Ne pas cacher si isVisible est true
    if (!isVisible) {
      _position = null;
  ...
  /// Méthode améliorée pour forcer l'affichage du crosshair et s'assurer qu'il reste visible
  void forceShow(dynamic x, double y, [String coordinateUnit = 'point']) {
    final dynamic parent = parentBox;
    if (parent == null) {
      return;
    }
    ...
    // Utiliser la méthode show standard pour positionner le crosshair
    show(x, y, coordinateUnit);

    // Annuler tout timer de masquage pour empêcher que le crosshair ne disparaisse
    if (_crosshairHideTimer != null) {
      _crosshairHideTimer!.cancel();
      _crosshairHideTimer = null;
    }

    // Force a refresh of the rendering if accessible
    try {
      if (parent.invalidate != null) {
        parent.invalidate();
      }
    } catch (e) {
      // Ignorer l'erreur si invalidate n'est pas accessible
    }
  }
  ...
  bool _actuallyVisible = true;  // Commence à true, comme isVisible

// Ajouter ce getter
  bool get actuallyVisible => _actuallyVisible;
  ...
  void onPaint(PaintingContext context, Offset offset,
    ...
    // Ne pas dessiner si isVisible est false
    if (!isVisible) {
      return;
    }
    ...
    if (parent.paintBounds.contains(_position!)) {
      ...
      // Si le crosshair est hors limites, il n'est pas visible
      _actuallyVisible = false;  // Mettre à jour l'état réel

      // Le reste du code pour nettoyer les données...
      _position = null;
      _resetCrosshairHolders();

      // Au lieu d'appeler invalidate(), programmez une mise à jour pour le prochain frame
      WidgetsBinding.instance.addPostFrameCallback((_) {
        final currentParent = parentBox as RenderBehaviorArea?;
        if (currentParent != null) {
          currentParent.invalidate();
        }
      });

      print('Crosshair hors limites - données effacées');
    }
  }
